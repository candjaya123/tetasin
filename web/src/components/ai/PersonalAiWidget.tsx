...
export function PersonalAiWidget() {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState<{role: 'ai' | 'user', content: string}[]>([
    { role: 'ai', content: 'Halo! Saya asisten finansial pribadi Anda. Apa rencana hemat kita hari ini?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/ai/personal/chat`, {
        method: 'POST',
        headers: {
