...
export default function IncomePage() {
  ...
  const [selectedAssetAccount, setSelectedAssetAccount] = useState(''); // Where the money goes (e.g. Cash)
  const [selectedIncomeAccount, setSelectedIncomeAccount] = useState(''); // Source of income (e.g. Salary)
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const transData = await journalService.getTransactions({ type: 'income' });
  ...
  const handleSave = async () => {
    if (!amount || !selectedAssetAccount || !selectedIncomeAccount) {
    ...
    try {
      ...
      // Debit Asset (Kas/Bank), Credit Revenue (Pendapatan)
      const payload = {
        reference_number: `INC-${Date.now()}`,
        description: description || 'Pemasukan',
        date: new Date().toISOString(),
        lines: [
          { account_id: selectedAssetAccount, debit: val, credit: 0 },
          { account_id: selectedIncomeAccount, debit: 0, credit: val }
        ]
      };
      ...
      toast({ title: "Pemasukan dicatat!", description: "Data Anda telah diperbarui." });
      setOpen(false);
      resetForm();
      fetchInitialData();
    } catch (error) {
  ...
  const incomeAccounts = coa.filter(a => ['revenue', 'income', 'pendapatan'].includes(a.type?.toLowerCase()) || a.code?.startsWith('4'));
    ...
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground font-medium mt-1">Catat seluruh dana yang masuk ke kantong Anda.</p>
        </div>
        
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger render={
            <Button className="h-14 px-8 rounded-2xl bg-green-600 hover:bg-green-700 text-white font-black shadow-xl shadow-green-100 flex items-center gap-3" />
          }>
            <Plus className="w-5 h-5" /> Catat Pemasukan
          </DialogTrigger>
          <DialogContent className="rounded-[2rem] p-10 max-w-md">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black text-green-600">Tambah Pemasukan</DialogTitle>
            </DialogHeader>
            <div className="space-y-6 mt-6">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="h-14 font-black text-2xl border-2 border-slate-100 rounded-xl focus:border-green-500"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label className="font-bold text-xs uppercase tracking-widest text-slate-400">Simpan Ke</Label>
                <select 
                  className="w-full h-14 px-4 rounded-xl border-2 border-slate-100 font-bold outline-none focus:border-green-500"
              ...
              <div className="space-y-2">
                <Label className="font-bold text-xs uppercase tracking-widest text-slate-400">Sumber Pemasukan</Label>
                <select 
                  className="w-full h-14 px-4 rounded-xl border-2 border-slate-100 font-bold outline-none focus:border-green-500"
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  className="h-14 font-medium border-2 border-slate-100 rounded-xl focus:border-green-500"
                  value={description}
              ...
              <Button 
                onClick={handleSave} 
                disabled={saving}
                className="w-full h-14 rounded-xl bg-green-600 text-white font-black mt-4 hover:bg-green-700"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="md:col-span-1 space-y-6">
           <Card className="border-none bg-green-600 text-white rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16 blur-2xl"></div>
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-4">Total Pemasukan</h4>
              ...
              <div className="space-y-6">
                 {transactions.slice(0, 5).map((t, i) => (
                   <div key={i} className="flex justify-between items-start border-b border-slate-100 pb-4 last:border-0 last:pb-0">
                      ...
                      <p className="text-xs font-black text-green-600">+{formatCurrency(t.total_amount)}</p>
                   </div>
                 ))}
        ...
        <div className="md:col-span-3">
          <Card className="border border-border bg-white rounded-[2.5rem] shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between">
               <h3 className="font-black text-secondary">Daftar Pemasukan Terakhir</h3>
                  ...
                  {loading ? (
                    ...
                    transactions.map((t, i) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-8 py-6 text-right font-black text-green-600">
