import { Controller, Post, Body, UseGuards, Request, UseInterceptors, UploadedFile, HttpException, HttpStatus, InternalServerErrorException, Logger } from '@nestjs/common';
...
export class AiController {
  ...
  @Post('business/chat')
  async businessChat(@Body() body: { prompt: string }, @Request() req: AuthenticatedRequest) {
    const { prompt } = body;
  ...
  @Post('personal/chat')
  async personalChat(@Body() body: { prompt: string }, @Request() req: AuthenticatedRequest) {
    const { prompt } = body;
    const tenantId = req.user.tenant_id;
    ...
    try {
      ...
      try {
        ...
        Logger.warn('Failed to fetch AI budget context:', e);
      }
