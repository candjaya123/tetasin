...
final reportSummaryProvider = FutureProvider<ReportSummary>((ref) async {
  ...
  return ref.watch(reportServiceProvider).getSummary(start: range.start, end: range.end);
});

final journalProvider = FutureProvider<List<JournalEntry>>((ref) async {
  final range = ref.watch(reportDateRangeProvider);
  return ref.watch(reportServiceProvider).getJournals(start: range.start, end: range.end);
});

final salesReportProvider = FutureProvider<List<SalesReportItem>>((ref) async {
  final range = ref.watch(reportDateRangeProvider);
  return ref.watch(reportServiceProvider).getSales(start: range.start, end: range.end);
});
...
final ledgerProvider = FutureProvider.family<List<LedgerEntry>, String>((ref, accountId) async {
  final range = ref.watch(reportDateRangeProvider);
  return ref.watch(reportServiceProvider).getLedger(accountId, start: range.start, end: range.end);
});

final selectedLedgerAccountProvider = StateProvider<String?>((ref) => null);

final incomeStatementProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  final range = ref.watch(reportDateRangeProvider);
  return ref.watch(reportServiceProvider).getIncomeStatement(
    range.start.toIso8601String(),
    range.end.toIso8601String(),
  );
});

final balanceSheetProvider = FutureProvider<List<BalanceSheetItem>>((ref) async {
