...
class AiChatNotifier extends StateNotifier<List<ChatMessage>> {
  ...
  AiChatNotifier(this._ref) : super([
    ChatMessage(text: 'Halo! Saya CFO Virtual Anda. Ada yang bisa saya bantu terkait laporan keuangan atau operasional toko hari ini?', isUser: false),
  ]);
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      if (e.response?.statusCode == 429) {
        errorMsg = '⏳ Kuota AI sedang tinggi. Mohon tunggu beberapa menit dan coba lagi.';
      } else if (e.response?.statusCode == 403) {
        errorMsg = '🔒 Fitur ini memerlukan akun dengan tier yang lebih tinggi.';
      } else if (e.type == DioExceptionType.connectionTimeout || e.type == DioExceptionType.receiveTimeout) {
        errorMsg = '🌐 Koneksi timeout. Pastikan internet Anda stabil.';
      } else {
        errorMsg = 'Maaf, terjadi kesalahan (${e.response?.statusCode ?? "network"}). Coba lagi nanti.';
      }
      state = [...state, ChatMessage(text: errorMsg, isUser: false)];
    } catch (e) {
      state = [...state, ChatMessage(text: 'Maaf, terjadi kesalahan tidak terduga. Silakan coba lagi.', isUser: false)];
    } finally {
      isLoading = false;
      state = [...state];
    }
